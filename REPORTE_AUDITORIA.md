...
**Problema**: `guardar_resultados_batch()` llamado desde thread secundario crasheaba con `ProgrammingError: SQLite objects created in a thread can only be used in that same thread`.

**Solución**: Agregado `check_same_thread=False` a `sqlite3.connect()`.

### 1.2 `app.py:343` — `subprocess` no importado
**Problema**: `sincronizar_sistema()` captura `subprocess.SubprocessError` pero `subprocess` nunca se importó en `app.py`. Lanza `NameError` no capturable.
